...
local function handleButtonEvent( event )
    ...
    if ( "ended" == event.phase ) then
        ...
        currentSpies = values[3].value --can be integer or string ("random" or a number)
		...
		myData.numOfSpies=values[3].value
	    ...
	    composer.gotoScene( "view_roles")--, sceneChangeOptions) --transport this data to the next scene
...
function scene:create( event )
	...
	local mySpyTextBox = display.newText( "#Spies", display.contentCenterX+110, 110, native.systemFont, 18 )
